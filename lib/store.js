...
export const calcVolume = (type, radius, height) => {
  ...
  switch (type) {
    case 'sphere':     return (4/3) * 3.14 * r * r * r
    case 'hemisphere': return (2/3) * 3.14 * r * r * r
    case 'cylinder':   return 3.14 * r * r * h
    case 'cone':       return (1/3) * 3.14 * r * r * h
    default:           return 0
  }
}
  ...
  persist(
    (set, get) => ({
      ...
      addShape: (type, radius, height) => {
        ...
        const r = parseFloat(radius)
        const h = parseFloat(height)
        const vol = calcVolume(type, r, h).toFixed(2)
        ...
        return { ok: true, shape: newShape, id }
      },
